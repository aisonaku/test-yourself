export default function() {
   ...
   this.get('/quizzes', function(db, request) {
      if(request.queryParams.category_id !== undefined) {
         let filteredData = data.filter(function(item) {
            return item.attributes.category.data.id == request.queryParams.category_id;
         });
         return {
            data: filteredData
         };
   ...
   var qData1= [{
      ...
      "type": "questions",
      "attributes": {
         "text": "Среди ночи Пират поднялся и медле(1)о вышел из сеней. Он долго стоял на пороге облитый серебря(2)ым лу(3)ым светом и напряже(4)о слушал. Но даже настороже(5)ый волчий слух не мог уловить ни малейшего признака жизни вокруг.\r\n\r\nВ каком варианте ответа правильно указаны все цифры, на месте которых пишется НН?",
         "answers": {
            "data": [{
               "id": "1",
               "type": "answers",
   ...
   this.get('/questions', function(db, request) {
      if(request.queryParams.quiz_id !== undefined) {
         let filteredData = data.filter(function(item) {
            return item.attributes.category.data.id == request.queryParams.quiz_id;
         });
         return {
            data: qData
         };
